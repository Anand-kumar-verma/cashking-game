export const green = "#47C45D"
export const yellow = "#DCB86A"
export const menhdi = "#8f5206"
// aviator color
export const gray = "!bg-[#151617]"
export const graydark = "!bg-[#1F2937]"
export const graymedium = "!bg-gray-500"
export const red = "#BC0319"
// main changes rows
export const bgcolorgradient = '#FE0000';
export const darktext = '#8f5206 !important'
export const zubgback = '#FFF';
// export const zubgtext = '#F02257';
export const zubgtext = 'linear-gradient(90deg, #ce0204 0%, #f2403a 100%)';
export const zubggray = '#6CB4EE';
export const zubgshadow = ' rgba(149, 157, 165, 0.2) 0px 8px 24px';
export const zubgwhite = 'linear-gradient(180deg,#F6E3A3 0%,#D2A753 100%)';
export const zubgback_cricket = '#3883BF';
export const zubgbackgrad = '#ff4b00c4';
// export const zubgbackgrad = 'linear-gradient(0deg, rgb(255 0 0) 0%, rgb(129, 0, 0) 100%)';
export const zubgmid = '#0D0335';
export const iconcoloryellow = '#FBB13B';
export const iconcolorpink = '#EE1285';
export const zubgcrickblue = '#613EF0';
export const zubgcrickorange = '#00A260';
export const zubgcrickgred = "#005543";
export const zubgcricklight = "#00B55E";
export const lightgreen = "#8BE68B";
export const lightblue = "#fff";
export const lightyellow = "#63BA0E";



export const bggray = '#757986';
export const bgdarkgray = '#fff';
export const bglightgray = '#E7E7E7';
export const bgfootergray = '#3B3B3B';
export const bggold = '#D91414';
export const bgtan = '#fff';
export const bggrad = 'linear-gradient(90deg, #ce0204 0%, #f2403a 100%)';
export const bgcolorlight = 'linear-gradient(90deg, #ce0204 0%, #f2403a 100%)';
export const bgcolorlight2 = '#e7e7e7 !important'
export const bggoldtext = '#8f5206 !important'

